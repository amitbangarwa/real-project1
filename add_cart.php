...
$cn=mysql_connect("localhost","root","") or die("Error in connecting database");
$db=mysql_select_db("art_gallery",$cn);
$sql="select * from gallery where picid=$pid";
$rs=mysql_query($sql,$cn);
$rsitem=mysql_fetch_object($rs);
if ($rsitem->status=="Incart")
{
echo"The Product Is In Your Cart<br><br>";
echo "<a href='view_all_galleries.php'><img src='image/button_continue.gif' alt='Back' title=' Back ' border='0' width='100' height='22'></a>";
}
if ($rsitem->status=="sold")
{echo"The Product Is Sold Out<br><br>";
echo "<a href='view_all_galleries.php'><img src='image/button_continue.gif' alt='Back' title=' Back ' border='0' width='100' height='22'></a>";
}
else
{
$picd=$rsitem->picid;
$astnm=$rsitem->artistname;
$picnm=$rsitem->picname;
$price=$rsitem->price;
$imgnm=$rsitem->img_name;
$rs2=mysql_query("UPDATE gallery SET status='Incart' where picid='$pid'");
$sql1="INSERT INTO cart(pid,artistnm,printnm,price,img_nm,status) VALUES ('$picd','$astnm','$picnm','$price','$imgnm','Incart')";
